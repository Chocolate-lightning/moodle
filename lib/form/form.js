    ...
    Y.extend(dependencyManager, Y.Base, {
        ...
        checkDependencies: function(e, dependon) {
            var dependencies = this.get('dependencies'),
                ...
                tolock = {},
                toshow = {},
                condition, value, isHide, lock, hide, show,
                ...
                for (value in dependencies[dependon][condition]) {
                    for (isHide in dependencies[dependon][condition][value]) {
                        ...
                        for (var ei in dependencies[dependon][condition][value][isHide]) {
                            ...
                            if (({}).hasOwnProperty.call(toshow, eltolock)) {
                                toshow[eltolock] = toshow[eltolock] || show;
                            } else {
                                toshow[eltolock] = show;
                            }
                        }
                    }
                }
            ...
            for (var el in tolock) {
                ...
                // TODO: MDL-81410 Tests labeled as MDL-81410 pass if the following is commented out. MDL-81410-2 always fail.
                if (!({}).hasOwnProperty.call(this._shows, el)) {
                    this._shows[el] = {};
                }
                if (({}).hasOwnProperty.call(toshow, el) && toshow[el]) {
                    if (!({}).hasOwnProperty.call(this._shows[el], dependon) || this._shows[el][dependon]) {
                        this._shows[el][dependon] = true;
                        needsupdate = true;
                    }
                } else if (({}).hasOwnProperty.call(this._shows[el], dependon) && this._shows[el][dependon]) {
                    delete this._shows[el][dependon];
                    needsupdate = true;
                }
        ...
        updateForm: function() {
            ...
            for (el in this._dirty) {
                ...
                // This will hide the element as long as everything in _shows[el] is false.
                // If even one item is true, it will not hide or lock the element.
                if (({}).hasOwnProperty.call(this._shows, el)) {
                    // TODO: Matt dirty hack.
                    if (el != 'allowedattempts[maxattemptsnum]') {
                        this._hideElement(el, Y.Object.isEmpty(this._shows[el]));
                        this._disableElement(el, Y.Object.isEmpty(this._shows[el]));
                    }
                }
            }
        ...
        _dependencyNotchecked: function(elements, value, isHide) {
            ...
            return {
                lock: lock,
                hide: isHide == 1 ? lock : false,
                show: isHide == 2 ? lock : false
            };
        },
