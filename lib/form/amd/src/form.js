...
export default class Form {
    ...
    domDispatch(elNamesObj) {
        this.elementNamesToDomNodes(elNamesObj.hide).forEach((element) => {
            if (element === null) {
                return;
            }
            MutateDom.hide(element);
        });
        this.elementNamesToDomNodes(elNamesObj.lock).forEach((element) => {
            if (element === null) {
                return;
            }
            MutateDom.disable(element);
        });
        this.elementNamesToDomNodes(elNamesObj.show).forEach((element) => {
            if (element === null) {
                return;
            }
            MutateDom.show(element);
        });
        this.elementNamesToDomNodes(elNamesObj.unlock).forEach((element) => {
            if (element === null) {
                return;
            }
            MutateDom.enable(element);
        });
    }
