    ...
    registerEventListeners() {
        ...
        const observer = new MutationObserver(async(mutationList) => {
            for (const mutation of mutationList) {
                ...
                if (mutation.type === "attributes" && this.dependencies.has(mutation.target.name)) {
                    const pendingPromise = new Pending('update');
                    // Look up any rules associated with the returned items.
                    this.domDispatch(this.displayMapPrune(this.dispatchDependencyRules(mutation.target)));
                    pendingPromise.resolve();
                }
            }
        });
        ...
        observer.observe(this.form, {attributes: true, subtree: true});
    }
