...
Feature: Within the grader report, test that we can search for users
  ...
  Scenario: A teacher can set focus and search using the input are with a keyboard
    ...
    And I wait until "Turtle Manatee" "menuitem" exists
    And I press the down key
    And the focused element is "Student 1" "menuitem"
    And I press the end key
    And the focused element is "Turtle Manatee" "menuitem"
    ...
    And I press the tab key
    And the focused element is "Filter by name" "button"
    ...
    And I set the field "Search users" to "User"
    And I click on ".usersearchwidget #searchinput" "css_element"
    And I press the enter key
  ...
  # TODO: The search debounce can find no users but if the all results is selected it can find users due to pagination.
