...
class grade_report_grader extends grade_report {
    ...
    public function get_left_rows(bool $displayaverages) : array {
        ...
        foreach ($extrafields as $field) {
            ...
            $collapsecontext = ['field' => $field, 'classes' => 'd-none'];
            $classes = '';
            $hidden = 'false';
            ...
            $singleview = $this->get_cell_action_menu([], 'userinfo', $classes, $hidden);
            $fieldheader->text = $arrows[$field] . $singleview . $collapsedicon;
        ...
        foreach ($this->gtree->get_levels() as $row) {
            ...
            foreach ($row as $element) {
                ...
                if ($type == 'filler' || $type == 'fillerfirst' || $type == 'fillerlast') {
                    ...
                    $hidden = '';
                    $collapsed = '';
                    $collapsecontext = ['field' => $element['object']->id, 'classes' => 'd-none'];
    ...
    public function get_cell_action_menu(array $element, string $mode, ?string $classes = '', ?string $hidden = 'false'): string {
        ...
        if (!empty($USER->editing) || isset($context->gradeanalysisurl)
            || isset($context->singleviewreporturl)  || isset($context->gradesonlyurl) || isset($context->columncollapse)) {
