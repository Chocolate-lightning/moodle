        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                if ($isstandard = $plugin->is_standard()) {
                    ...
                    $sourcelabel = html_writer::span(get_string('sourceext', 'core_plugin'), 'sourcetext tag tag-info');
                }
                ...
                switch ($statuscode) {
                    case core_plugin_manager::PLUGIN_STATUS_NEW:
                        $statusclass .= $dependenciesok ? 'tag-success' : 'tag-warning';
                        break;
                    case core_plugin_manager::PLUGIN_STATUS_UPGRADE:
                        $statusclass .= $dependenciesok ? 'tag-info' : 'tag-warning';
                        break;
                    ...
                    case core_plugin_manager::PLUGIN_STATUS_UPTODATE:
                        $statusclass .= $dependenciesok ? '' : 'tag-warning';
                        break;
                }
        ...
        foreach ($dependencies as $plugin) {
            ...
            foreach ($plugin->version->supportedmoodles as $moodle) {
                if ($CFG->branch == str_replace('.', '', $moodle->release)) {
                    $supportedmoodles[] = html_writer::span($moodle->release, 'tag tag-success');
                } else {
        ...
        foreach ($pluginman->resolve_requirements($plugin, $version) as $reqname => $reqinfo) {
            if ($reqname === 'core') {
                if ($reqinfo->status == $pluginman::REQUIREMENT_STATUS_OK) {
                    ...
                    $label = html_writer::span(get_string('dependencyfails', 'core_plugin'), 'tag tag-danger');
                }
                ...
                if ($reqinfo->status == $pluginman::REQUIREMENT_STATUS_OK) {
                    ...
                    if ($reqinfo->availability == $pluginman::REQUIREMENT_AVAILABLE) {
                        $label = html_writer::span(get_string('dependencymissing', 'core_plugin'), 'tag tag-warning');
                        $label .= ' '.html_writer::span(get_string('dependencyavailable', 'core_plugin'), 'tag tag-warning');
                        ...
                        $label = html_writer::span(get_string('dependencymissing', 'core_plugin'), 'tag tag-danger');
                    ...
                    if ($reqinfo->availability == $pluginman::REQUIREMENT_AVAILABLE) {
                        $label = html_writer::span(get_string('dependencyfails', 'core_plugin'), 'tag tag-warning');
                        $label .= ' '.html_writer::span(get_string('dependencyavailable', 'core_plugin'), 'tag tag-warning');
        ...
        foreach ($plugininfo as $type => $plugins) {
            ...
            foreach ($plugins as $name => $plugin) {
                ...
                if ($plugin->is_standard()) {
                    ...
                    $row->attributes['class'] .= ' extension';
                    $source = html_writer::div(get_string('sourceext', 'core_plugin'), 'source tag tag-info');
                }

                if ($status === core_plugin_manager::PLUGIN_STATUS_MISSING) {
                    $msg = html_writer::div(get_string('status_missing', 'core_plugin'), 'statusmsg tag tag-danger');
                } else if ($status === core_plugin_manager::PLUGIN_STATUS_NEW) {
                    $msg = html_writer::div(get_string('status_new', 'core_plugin'), 'statusmsg tag tag-success');
                } else {
                    $msg = '';
                }
