    ...
    events.forEach((event) => {
        document.addEventListener(event, async(e) => {
            if (e.target.closest(selectors.elements.sectionmodchooser)) {
                ...
                const favouriteFunction = partiallyAppliedFavouriteManager(data);
                ...
                ChooserDialogue.displayChooser(caller, sectionModal, builtModuleData, courseId, favouriteFunction);
            }
        });
    });
};
...
const partiallyAppliedFavouriteManager = builtModules => {
    ...
    return async(internal, favourite, modalBody) => {
        ...
        const result = builtModules.content_items.find(({name}) => name === internal);
        if (result) {
            if (favourite) {
                result.favourite = true;

                const newFaves = builtModules.content_items.filter(mod => mod.favourite === true);
                const {html, js} = await Templates.renderForPromise('core_course/chooser_favourites', {favourites: newFaves});
